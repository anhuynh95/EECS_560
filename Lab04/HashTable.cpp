...
void HashTable::Insert(int x)
{
	if(table[Hash(x)].isHere(x) == false)
	{
		...
		if(lamda < 1)
		{
			...
			cout<<"Open Hashing: :"<<x<<" is inserted into the hash table\n";
			intNum++;
		}
		else
		{
	...
	{
	 	cout<<"Open Hashing: :"<<x<<"  is a duplicate of "<<x<<" can’t be added to the hash table\n";
	}
}
...
bool HashTable::Find(int x)
{
	...
	if(table[Hash(x)].isHere(x) == true)
	{
		cout<<"Open Hashing: "<<x<<" is found at location "<<position<<"\n";
		return true;
	}
	else
	{
		cout<<"Open Hashing: "<<x<<"  can’t be found in the hash table.\n";
		return false;
	}
}
