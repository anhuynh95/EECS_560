...
bool BinarySearchTree::myInsert(int val, BinaryNode* tree) {
	if(IsEmpty() == true)
	{
	...
	else if(val < tree->getValue())
	{
		...
		{
			return (myInsert(val, tree->getLeft()));
		}
	}
	else if (val >= tree->getValue())
	{
		...
		{
			return(myInsert(val, tree->getRight()));
		}
	}
	else return false;
}
...
void BinarySearchTree::myLevelorder (BinaryNode* tree, int level) {
	if(tree == nullptr) return;
	else if(level == 1) cout<<tree->getValue()<<" ";
	else if(level > 1)	{
